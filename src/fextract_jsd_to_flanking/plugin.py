...
class FextractHooks:
    ...
    def transform_all_intervals(self, single_intervals_transformed_reads: Signal, config: Any,
        ...
        M = lambda x: (flanking_distribution + x) / 2
        jds_to_flanking = np.apply_along_axis(lambda x: 0.5 * (scipy.stats.entropy(x, M(x)) +
                                                               scipy.stats.entropy(flanking_distribution, M(x))),
                                              0,
                                              single_intervals_transformed_reads.array)
        kl_f = np.apply_along_axis(lambda x: scipy.stats.entropy(x, flanking_distribution), 0,
                                   single_intervals_transformed_reads.array)
        kl_r = np.apply_along_axis(lambda x: scipy.stats.entropy(flanking_distribution, x), 0,
                                   single_intervals_transformed_reads.array)
        return Signal(array=jds_to_flanking,
                      tags=("jsd_to_flanking",),
                      metadata={"kl_f": kl_f,
                                "kl_r": kl_r,
    ...
    def plot_signal(self, signal: Signal,
        ...
        with plt.style.context('seaborn-v0_8-whitegrid'):
            ...
            sns.heatmap(signal.metadata["array"], ax=ax[0], cmap="viridis", rasterized=True, cbar=False)
            ax[1].set_title(f"{signal_type}\n"
                            f"patient: {extra_config.ctx['path_to_bam'].stem} "
                            f"bed file: {extra_config.ctx['path_to_bed'].stem.split('.', 1)[0]}", fontsize=20)
            fig, _ = plot_signal(signal.array, apply_savgol=False, ax=ax[1], fig=fig, label=signal_type)
            fig, _ = plot_signal(signal.metadata["kl_f"], ax=ax[2], fig=fig, label="kl_f")
            fig, _ = plot_signal(signal.metadata["kl_r"], ax=ax[3], fig=fig, label="kl_r")
            fig.tight_layout()
            ax[1].set_ylabel(signal_type)
            ax[1].set_xlabel("Position")

        fig.savefig(
            extra_config.ctx["output_path"] /
            f"{generate_time_stamp()}__{extra_config.ctx['id']}__{signal_type}_signal_plot.png",
            dpi=300)
        return fig
...
class CliHook:
    r"""
        This CliHook implements the CLI interface for the extract_jsd_to_flanking feature extraction method.
        ...
        **extract_jsd_to_flanking**
        ...
        where :math:`M = \frac{1}{2} (P_l + Q)` is the average distribution of :math:`P` at position `l` and :math:`Q`, and 
        :math:`\text{KLD}` is the Kullback–Leibler divergence defined by:
    ...
    def get_command(self) -> click.Command | List[click.Command]:
        ...
        def extract_jsd_to_flanking(
            ...
            res = App(plugins_name=["jsd_to_flanking",
                                    "relative_entropy_to_flanking",
        ...
        return extract_jsd_to_flanking
